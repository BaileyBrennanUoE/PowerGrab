...
public class Drone {
    ...
    public void updateCharge(BigDecimal stationCharge) {
        ...
        charge = Math.max(0, (preciseCharge.add(stationCharge).subtract(moveCost)).doubleValue()); // Drone cannot store
                                                                                                   // debt
    }

    // Runs the deciding process on where to make the next move for the stateless
    // drone
    public Direction decideMove(List<Pair> goodStations, List<Pair> badStations, List<Pair> neutralPoints) {
    ...
    public Direction decideMoveStateful(List<Pair> goodStations, List<Pair> badStations, List<Pair> neutralPoints, Position nearestStation) {
    int index = 0;
    Direction d = Direction.N;
    
    if (!goodStations.isEmpty()) {
        for(int i=0;i<goodStations.size();i++) {
            Position goodStation=goodStations.get(i).getPos();
            if(goodStation.equals(nearestStation)) {
                d = findDirection(currentPos, nearestStation);
                break;
            }
        }
        //d = asDirection(goodStations.get().getDirection());
    } else if (!neutralPoints.isEmpty()) {
        index = rnd.nextInt(neutralPoints.size());
        d = asDirection(neutralPoints.get(index).getDirection());
    } else {
    ...
    public Direction findDirection(Position currentPos, Position nearestStation) {
        Direction d = Direction.N;
